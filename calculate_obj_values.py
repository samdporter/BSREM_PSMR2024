# Description: This script calculates the objective function values for VTV
# Mostly copied from main so will have some uneeeded imports
...
parser = argparse.ArgumentParser(description='calculate_obj_values')
...
parser.add_argument('--output_path', type=str, default="/home/sam/working/BSREM_PSMR_MIC_2024/results/test/stoch_long", help='output path')
...
parser.add_argument('--image_path', type=str, default='/home/sam/working/BSREM_PSMR_MIC_2024/images', help='image path')
parser.add_argument('--base_filename', type=str, default='BSREMmm_of_', help='base filename')
...
def find_objective_functions_in_directory(file_path, file_name_base, obj_fun, prior):
    files = os.listdir(file_path)
    files = [f for f in files if file_name_base in f and ".hv" in f]   

    pet_files = [f for f in files if "image_0" in f]
    spect_files = [f for f in files if "image_1" in f]
    
    # Sort images by iteration number
    iteration_p = [int(image.split("_")[2]) for image in pet_files]
    iteration_s = [int(image.split("_")[2]) for image in spect_files]
    pet_files = [x for _, x in sorted(zip(iteration_p, pet_files))]
    spect_files = [x for _, x in sorted(zip(iteration_s, spect_files))]
    iteration_p = sorted(iteration_p)
    iteration_s = sorted(iteration_s)
    assert iteration_p == iteration_s
    
    objs = []
    datas = []
    priors = []
    
    for i, p, s in zip(iteration_p, pet_files, spect_files):
        pet_file = os.path.join(file_path, p)
        spect_file = os.path.join(file_path, s)
        
        print("pet_file", pet_file)
        print("spect_file", spect_file)
        
        o, d, p = get_objective_function_values(i, pet_file, spect_file, obj_fun, prior)
        objs.append(o)
        datas.append(d)
        priors.append(p)
    
    return objs, datas, priors

def get_objective_function_values(iteration, pet_file, spect_file, obj_fun, prior):
    pet_image = ImageData(pet_file)
    spect_image = ImageData(spect_file)
    image = BlockDataContainer(pet_image, spect_image)
    
    d = obj_fun(image)
    p = prior(image)
    o = d - p
    
    print(f"Iteration {iteration}: Objective function value: {o}")
...
def main(args):
    ...
    return find_objective_functions_in_directory(args.image_path, args.base_filename, SIRFBlockFunction([pet_obj_fun, spect_obj_fun]), prior)
...
if __name__ == "__main__":
    ...
    obj, data, prior = main(args)
    
    if not os.path.exists(args.output_path):
        os.makedirs(args.output_path)
        
    # save the objective function values to a csv file
    df_full = pd.DataFrame(obj)
    df_full.to_csv(os.path.join(args.output_path, f"bsrem_objective_a_{args.alpha}_b_{args.beta}.csv"))
    ...
    df_full = pd.concat([df_full, df_data, df_prior], axis=1)
    # add column names
